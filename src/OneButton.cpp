 ...
 *   Copyright (c) Matthias Hertel, https://www.mathertel.de.
 *   Copyright (c) Ihor Nehrutsa, Ihor.Nehrutsa@gmail.com
 *   http://www.mathertel.de/License.aspx
 *
 * Modified work:
 *   Copyright (c) 2026 uminus
 *
...
OneButton::OneButton(const int pin, const bool activeLow, const bool pullupActive) {
  setup(pin, pullupActive ? GPIO_CNF_IN_PUPD : GPIO_CNF_IN_FLOATING, activeLow);
}  // OneButton
...
void OneButton::setup(const uint8_t pin, const uint8_t mode, const bool activeLow) {
  ...
  if (activeLow) {
    ...
    _buttonPressed = FUN_LOW;

  } else {
    // the button connects the input pin to VCC when pressed.
    _buttonPressed = FUN_HIGH;
  }
...
void OneButton::tick(void) {
  if (_pin >= 0) {
    _fsm(debounce(funDigitalRead(_pin) == _buttonPressed));
  }
}  // tick()
